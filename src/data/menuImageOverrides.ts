// Drink image overrides. Legacy images remain untouched and are still the fallback.
// Brand-specific items use matching product photography where possible; generic/private-label
// drinks use illustrative photography of the same drink type rather than another brand.
export const menuImageOverrides: Record<string, string> = {
  // المشروبات الغازية
  "pepsi-cans":
    "https://images.pexels.com/photos/30554816/pexels-photo-30554816.jpeg?cs=srgb&fm=jpg",
  "free-cola":
    "https://images.pexels.com/photos/11594548/pexels-photo-11594548.jpeg?auto=compress&cs=tinysrgb&w=800",
  "twist-classic":
    "https://images.pexels.com/photos/32658918/pexels-photo-32658918.jpeg?auto=compress&cs=tinysrgb&w=800",
  "pepsi-1-5l":
    "https://images.pexels.com/photos/988954/pexels-photo-988954.jpeg?cs=srgb&fm=jpg",
  "big-cola":
    "https://images.pexels.com/photos/27289727/pexels-photo-27289727.jpeg?auto=compress&cs=tinysrgb&w=800",

  // الألبان والمشروبات التقليدية
  "almarai-rayeb":
    "https://images.pexels.com/photos/26648792/pexels-photo-26648792.jpeg?auto=compress&cs=tinysrgb&w=800",
  "aswan-hibiscus":
    "https://images.pexels.com/photos/36630822/pexels-photo-36630822.jpeg?auto=compress&cs=tinysrgb&w=800",
  "sobia-milk":
    "https://images.pexels.com/photos/1933386/pexels-photo-1933386.jpeg?auto=compress&cs=tinysrgb&w=800",
  "tamarind":
    "https://images.pexels.com/photos/34467107/pexels-photo-34467107.jpeg?auto=compress&cs=tinysrgb&w=800",
  "kharoub":
    "https://images.pexels.com/photos/32658918/pexels-photo-32658918.jpeg?auto=compress&cs=tinysrgb&w=800",
  "sakans-mix":
    "https://images.pexels.com/photos/37228408/pexels-photo-37228408.jpeg?auto=compress&cs=tinysrgb&w=800",
};
